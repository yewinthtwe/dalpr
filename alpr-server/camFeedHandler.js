...
async function updateInOutRecordById(trafficToken) {
	...
	if (isExitLane) {
		...
		if (!isMember) {
			console.log(
				`updateInOutRecordById: ${currentLicensePlate} > Calculating Parking Fee....`
			);
			console.log(
				`updateInOutRecordById: ${currentLicensePlate} > In Time: ${oldInTime}`
			);
			console.log(
				`updateInOutRecordById: ${currentLicensePlate} > Out Time: ${currentTime}`
			);
			parkedMinutes = differenceInMinutes(currentTime, oldInTime);
			console.log(
				`updateInOutRecordById: ${currentLicensePlate} > Total parked minutes: ${parkedMinutes}`
			);
			parkingFee = parkedMinutes * rate;
			console.log(
				`updateInOutRecordById: ${currentLicensePlate} > Total parking fee: ${parkingFee}`
			);
			isUsed = true;
		}
		const updatedRecord = await InOutRecord.updateOne(
			{ _id: oldId },
			{
				$set: {
	...
	console.log(
		`updateInOutRecordById: ${currentLicensePlate} > Document updated.`
	);
}

async function searchLicensePlate(licensePlate) {
	console.log(
		`searchLicensePlate: ${licensePlate} > Searching in temporary database (InOutRecords).`
	);
	try {
		...
		const lpDoc = await InOutRecord.findOne({
			"candidates.plate": { $in: licensePlate },
			"ticket.isUsed": false,
		})
			.sort({ updatedAt: -1 })
			.limit(1);
		...
		if (!lpDoc) {
			console.log(
				`searchLicensePlate: ${licensePlate} > No matching record found in temporary database: (InOutRecords).`
			);
			return lpDoc;
		} else {
			console.log(
				`searchLicensePlate: ${licensePlate} > Found document ID: ${lpDoc._id} in temporaty database.`
			);
			return lpDoc;
		}
	} catch (error) {
