...
export async function saveMember(member, source) {
      try {
          ...
          if (member._id) {
            const body = {...member};
            delete body._id;
            const response = await http.put(memberUrl(member._id), body, { cancelToken: source.token });
            const {data} = response;
            return data;
          }
          const response = await http.post(apiEndpoint, member, { cancelToken: source.token });
          const {data } = response;
          return data;
       } catch (error) {
        if (http.isCancel(error)) {
...
export async function getObuCollection(source) {
  try {
    const response = await http.get(`${apiUrl}/obus`,{
      cancelToken: source.token,
    });
    const { data } = response;
